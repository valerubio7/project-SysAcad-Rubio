...
class OrientacionTestCase(unittest.TestCase):
    ...
    def test_orientacion_create(self):
        ...
        db.session.commit()
        plan = Plan(nombre="Plan X", fecha_inicio=datetime.date(2025, 1, 1), fecha_fin=datetime.date(2025, 12, 31), observacion="Obs Plan")
        db.session.add(plan)
        db.session.commit()
    ...
    def test_orientacion_read(self):
        ...
        db.session.add(especialidad)
        db.session.commit()
        plan = Plan(nombre="Plan Y", fecha_inicio=datetime.date(2026, 1, 1), fecha_fin=datetime.date(2026, 12, 31), observacion="Obs Plan Y")
        db.session.add(plan)
        db.session.commit()
    ...
    def test_orientacion_update(self):
        ...
        db.session.add(especialidad)
        db.session.commit()
        plan = Plan(nombre="Plan Z", fecha_inicio=datetime.date(2027, 1, 1), fecha_fin=datetime.date(2027, 12, 31), observacion="Obs Plan Z")
        db.session.add(plan)
        db.session.commit()
    ...
    def test_orientacion_delete(self):
        ...
        db.session.add(especialidad)
        db.session.commit()
        plan = Plan(nombre="Plan W", fecha_inicio=datetime.date(2028, 1, 1), fecha_fin=datetime.date(2028, 12, 31), observacion="Obs Plan W")
        db.session.add(plan)
        db.session.commit()
